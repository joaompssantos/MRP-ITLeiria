...
DECODER *init_decoder(FILE *fp, int **back_ref_error, int **for_ref_error, int width, int height, int maxval,
                      ...
                      int quadtree_depth, int delta, int depth, char *debug_path) {
    DECODER *dec;
    int i;
    ...
    dec->maxprd = (uint) dec->maxval << (uint) dec->coef_precision;
    dec->delta = delta;
    dec->depth = depth;

    if (debug_path != NULL) {
        int len = strlen(debug_path);
        dec->debug_path = (char *) alloc_mem((len + 1) * sizeof(char));
    ...
    dec->err_debug = (int **) alloc_2d_array(dec->height, dec->width, sizeof(int));
    ...
    if (dec->mi_prd_order[LEFT] > 0) {
        dec->mi_left_roff = init_ref_offset(dec->height, dec->width, MI_LEFT_PRED, dec->mi_prd_order[LEFT],
                                            dec->mi_size);
    ...
    if (dec->mi_prd_order[LDIAG] > 0) {
        dec->mi_ldiag_roff = init_ref_offset(dec->height, dec->width, MI_LDIAG_PRED, dec->mi_prd_order[LDIAG],
                                             dec->mi_size);
...
void free_decoder(DECODER *dec) {
    ...
    if (dec->mi_prd_order[LEFT] > 0) {
        ...
        free_ref_offset(dec->height, dec->width, MI_LEFT_PRED, dec->mi_prd_order[LEFT], dec->mi_size,
                        dec->mi_left_roff);
    }

    if (dec->mi_prd_order[LDIAG] > 0) {
        free(dec->mi_ldiag_ctx_weight);
        free_ref_offset(dec->height, dec->width, MI_LDIAG_PRED, dec->mi_prd_order[LDIAG], dec->mi_size,
                        dec->mi_ldiag_roff);
    }

    if (dec->mi_prd_order[RDIAG] > 0) {
        free(dec->mi_rdiag_ctx_weight);
        free_ref_offset(dec->height, dec->width, MI_RDIAG_PRED, dec->mi_prd_order[RDIAG], dec->mi_size,
                        dec->mi_rdiag_roff);
    }
...
void
decode_qtindex(FILE *fp, DECODER *dec, VLC *vlc, PMODEL *cpm, int tly, int tlx, int blksize, int width, int level) {
...
void debug_partition(DECODER *dec, IMAGE *img, int endianness) {
    int d, k, i, j, z;
    unsigned short byte;
    ...
    if (dec->debug_path != NULL) {
        ...
        int elements = dec->height * dec->width * 3 * (dec->depth == 8 ? 1 : 2);
        ...
        if (dec->quadtree_depth > 0) {
            ...
            for (d = dec->quadtree_depth - 1; d >= 0; d--) {
                for (i = 0; i < img->height; i += blksize) {
                    for (j = 0; j < img->width; j += blksize) {
                        for (k = 0; k < 2; k++) {
                            if ((dec->qtmap[d][i / blksize][j / blksize] == 0 && d == dec->quadtree_depth - 1) ||
                                ...
                                if (i + blksize - 1 < dec->height) {
                                    for (z = j; z < (j + blksize < dec->width ? j + blksize : dec->width); z++) {
                                        qt_img[k][i + blksize - 1][z] = (img_t) dec->maxval;
                                    }
                                }
                                if (j + blksize - 1 < dec->width) {
                                    for (z = i; z < (i + blksize < dec->height ? i + blksize : dec->height); z++) {
                                        qt_img[k][z][j + blksize - 1] = (img_t) dec->maxval;
                                    }
                                }
                            }
                            if (d == 0 && dec->qtmap[d][i / blksize][j / blksize] == 1) {
                                if (i + blksize / 2 - 1 < dec->height) {
                                    for (z = j; z < (j + blksize < dec->width ? j + blksize : dec->width); z++) {
        ...
        sprintf(partition_img, "%s/partition_%dx%d_%dbpp_LE_YUV444p.yuv", dec->debug_path, dec->width, dec->height,
                dec->depth);
...
void debug_residuals(DECODER *dec) {
    if (dec->debug_path != NULL) {
        ...
        for (i = 0; i < dec->height; i++) {
            for (j = 0; j < dec->width; j++) {
                if (dec->depth + 1 > 8) {
                    byte = reverse_endianness(dec->err_debug[i][j] + shift, LITTLE_ENDIANNESS);

                    *ptr++ = (byte >> 8u) & 0x00FFu;
                    *ptr++ = byte & 0x00FFu;
                }
                else {
                    *ptr++ = dec->err_debug[i][j] + shift;
                }
            }
        }

        sprintf(err_img, "%s/residuals_SAI_%dx%d_%dbpp_LE_GRAY_Y.yuv", dec->debug_path, dec->width, dec->height,
                dec->depth + 1);
        ...
        for (i = 0; i < dec->height; i++) {
            for (j = 0; j < dec->width; j++) {
                if (dec->depth + 1 > 8) {
                    byte = reverse_endianness(dec->err_debug[i][j] + shift, BIG_ENDIANNESS);
        ...
        sprintf(err_img, "%s/residuals_SAI_%dx%d_%dbpp_LE_GRAY_Y.pgm", dec->debug_path, dec->width, dec->height,
                dec->depth + 1);

        debug_fp = fopen(err_img, "wb");
        fprintf(debug_fp, "P5\n%d %d\n%d\n", dec->width, dec->height, (int) pow(2, dec->depth + 1) - 1);
        fwrite(err_stream, sizeof(unsigned char), elements, debug_fp);
        fclose(debug_fp);
        ...
        for (k = 0; k < dec->height / dec->mi_size[HEIGHT]; k++) {
            for (l = 0; l < dec->width / dec->mi_size[WIDTH]; l++) {
                for (i = k * dec->mi_size[HEIGHT]; i < (k + 1) * dec->mi_size[HEIGHT]; i++) {
                    for (j = l * dec->mi_size[WIDTH]; j < (l + 1) * dec->mi_size[WIDTH]; j++) {
                        if (dec->depth + 1 > 8) {
                            byte = reverse_endianness(dec->err_debug[i][j] + shift, LITTLE_ENDIANNESS);
        ...
        sprintf(err_img, "%s/residuals_PVS_%dx%d_%dbpp_LE_GRAY_Y.yuv", dec->debug_path, dec->mi_size[WIDTH], dec->mi_size[HEIGHT],
                dec->depth + 1);
...
int main(int argc, char **argv) {
    int i, f, **error = NULL;
    int width, height, maxval, frames, depth, bframes, bidirectional = 0;
    int num_comp, num_group, endianness = LITTLE_ENDIANNESS;
    ...
    while (f < frames) {
        ...
        if (f == 0) {
            ...
            dec = init_decoder(fp, error, NULL, width, height, maxval, num_comp, num_group, prd_order[1], prd_order[2],
                               ...
                               quadtree_depth, delta, depth, debug_path);
            ...
            if ((f - first_frame) % bframes == 0) {
                dec = init_decoder(fp, back_ref_error, NULL, width, height, maxval, num_comp, num_group, prd_order[1],
                                   ...
                                   f_huffman, quadtree_depth, delta, depth, debug_path);
            }
            else {
                dec = init_decoder(fp, back_ref_error, for_ref_error, width, height, maxval, num_comp, num_group,
                                   ...
                                   coef_precision, pm_accuracy, f_huffman, quadtree_depth, delta, depth, debug_path);
            }
        }
        ...
        dec->pmodels = init_pmodels(dec->num_group, dec->num_pmodel, dec->pm_accuracy, dec->pm_idx, dec->sigma,
                                    dec->maxval + 1);
        ...
        if (debug == 1) {
            ...
            debug_partition(dec, video[1], endianness);
            debug_residuals(dec);
        }
