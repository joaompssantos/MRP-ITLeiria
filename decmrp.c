...
DECODER *init_decoder(FILE *fp, int **back_ref_error, int **for_ref_error, int width, int height, int maxval,
                      int num_comp, int num_group, int prd_order, int back_prd_order, int for_prd_order, int num_pmodel,
                      int coef_precision, int pm_accuracy, int f_huffman, int quadtree_depth, int delta, int depth) {
    DECODER *dec;
    int i;
    ...
    dec->full_prd_order = dec->prd_order + dec->back_prd_order + dec->for_prd_order;
    ...
    dec->intra_roff = init_ref_offset(dec->height, dec->width, INTRA_PRED, dec->prd_order);
    ...
    if (dec->back_prd_order > 0) {
        dec->back_roff = init_ref_offset(dec->height, dec->width, BACK_PRED, dec->back_prd_order);
        dec->back_ctx_weight = init_ctx_weight(BACK_PRED, dec->back_prd_order, dec->delta);
    }

    if (dec->for_prd_order > 0) {
        dec->for_roff = init_ref_offset(dec->height, dec->width, FOR_PRED, dec->for_prd_order);
...
void free_decoder(DECODER *dec) {
    ...
    free_ref_offset(dec->height, dec->width, INTRA_PRED, dec->prd_order, dec->intra_roff);

    if (dec->back_prd_order > 0) {
        free(dec->back_ctx_weight);
        free_ref_offset(dec->height, dec->width, BACK_PRED, dec->back_prd_order, dec->back_roff);
    }

    if (dec->for_prd_order > 0) {
        free(dec->for_ctx_weight);
        free_ref_offset(dec->height, dec->width, FOR_PRED, dec->for_prd_order, dec->for_roff);
    }
...
void read_header(FILE *fp, int *width, int *height, int *maxval, int *frames, int *depth, int *bframes, int *num_comp,
                 int *num_group, int prd_order[6], int *num_pmodel, int *coef_precision, int *pm_accuracy,
                 int *f_huffman, int *quadtree_depth, int *delta, int *hevc, int *hist_bytes) {
    int i = 0;
...
int main(int argc, char **argv) {
    ...
    read_header(fp, &width, &height, &maxval, &frames, &depth, &bframes, &num_comp, &num_group, prd_order,
                &num_pmodel, &coef_precision, &pm_accuracy, &f_huffman, &quadtree_depth, &delta, &hevc, &hist_bytes);
    ...
    while(f < frames) {
        ...
        if (f == 0) {
            dec = init_decoder(fp, NULL, NULL, width, height, maxval, num_comp, num_group, prd_order[0], 0, 0,
                               num_pmodel, coef_precision, pm_accuracy, f_huffman, quadtree_depth, delta, depth);
        }
        else if (bidirectional == 0) {
            video[0] = video[1];

            dec = init_decoder(fp, error, NULL, width, height, maxval, num_comp, num_group, prd_order[1], prd_order[2],
                               0, num_pmodel, coef_precision, pm_accuracy, f_huffman, quadtree_depth, delta, depth);
            ...
            if ((f - first_frame) % bframes == 0) {
                dec = init_decoder(fp, back_ref_error, NULL, width, height, maxval, num_comp, num_group, prd_order[1],
                                   prd_order[2], 0, num_pmodel, coef_precision, pm_accuracy, f_huffman, quadtree_depth,
                                   delta, depth);
            }
            else {
                dec = init_decoder(fp, back_ref_error, for_ref_error, width, height, maxval, num_comp, num_group,
                                   prd_order[3], prd_order[4], prd_order[5], num_pmodel, coef_precision, pm_accuracy,
